...
const Home: NextPage = () => {
  ...
  const onClickResgistration = () => {
    router.push(`/register`)
  }

  const router = useRouter()
  return (
    <div className="main_container_home_temp   flex flex-col">
      <div className="container_logo h-auto  w-60">
        <p className="font-butler text-black mt-0 text-gold text-center text-xl">Bienvenido a</p>
      ...
      <div className="flex flex-col space-y-4 w-60 mt-20 ">
        <Button title="Iniciar Sesión" onClick={onClickLogin} customClassName="w-full h-7" />
        <Button title="Registrarse" isGold={true} onClick={onClickResgistration} customClassName="w-full h-7 background-white" />
      </div>
    </div>
  )
}
